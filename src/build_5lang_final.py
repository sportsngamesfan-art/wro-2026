...
CSV_PATH = "data/hindi_english_sanskrit_tamil_odia_phrases.csv"
...
with open(CSV_PATH, "r", encoding="utf-8-sig") as f:
    for row in csv.DictReader(f):
        ...
        rows.append({
            ...
            "od": row.get("odia_translation", "").strip() or e,
            "s": slug(e),
        })
...
<style>
  * { box-sizing: border-box; }
  ...
  .toggle button { padding: 10px 16px; margin: 4px 6px 4px 0; cursor: pointer; border: 2px solid #666; background: white; border-radius: 6px; font-size: 15px; }
  ...
  .btn { background: #2196F3; color: white; padding: 12px; border: none; border-radius: 6px; cursor: pointer; font-family: inherit; text-align: center; }
...
<header>
  ...
  <div class="toggle">
    ...
    <button id="btn-od" onclick="setLang('od')">ଓଡ଼ିଆ</button>
  </div>
</header>
<div class="bar"><input id="search" placeholder="Search..."><span id="count"></span></div>
...
function candidates(p) {
  ...
  if (lang === 'od') return ['audio/odia_' + p.s + '.wav'];
...
function textFor(p) {
  ...
  if (lang === 'od') return p.od;
  return p.e;
}
function subFor(p) { return lang === 'en' ? p.h : p.e; }
...
function attemptPlay() {
  if (!tryQueue.length) { nowPlaying.textContent += '  (audio not found)'; return; }
...
function setLang(l) {
  lang = l;
  ['hi','en','sa','ta','od'].forEach(x => document.getElementById('btn-'+x).classList.toggle('on', l===x));
  render();
}
function render() {
  ...
  let shown = 0;
  PHRASES.forEach(p => {
    if (q && p.h.toLowerCase().indexOf(q)===-1 && p.e.toLowerCase().indexOf(q)===-1 &&
        p.sa.toLowerCase().indexOf(q)===-1 && p.ta.toLowerCase().indexOf(q)===-1 &&
        p.od.toLowerCase().indexOf(q)===-1) return;
    shown++;
    const b = document.createElement('button');
    b.className = 'btn';
    b.innerHTML = '<div class="primary">'+textFor(p)+'</div><div class="secondary">'+subFor(p)+'</div>';
    b.onclick = () => playPhrase(p, b);
    grid.appendChild(b);
  });
...
with open("audio_player.html", "w", encoding="utf-8") as f:
    f.write(html)
print("Wrote audio_player.html with 5 languages — done.")